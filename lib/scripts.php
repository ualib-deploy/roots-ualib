 ...
 * 1. /roots-ualib/assets/css/main.min.css
 *
 * Enqueue scripts in the following order:
 * 1. jquery-1.10.2.min.js via Google CDN
 * 2. /roots/assets/js/vendor/modernizr-2.6.2.min.js
 * 3. /roots-ualib/assets/js/main.min.js (in footer)
 */
function ualib_scripts() {
	
	if ( ! ualib_is_dev_mode() ) {
	...
	$main_css = ( ualib_is_dev_mode() ) ? '/assets/css/main.css' : '/assets/dist' . $main_versioned_css;
	wp_enqueue_style( 'ualib_main', get_stylesheet_directory_uri() . $main_css, false, null );
	...
	if ( ! is_admin() && current_theme_supports( 'jquery-cdn' ) ) {
		...
		add_filter( 'script_loader_src', 'ualib_jquery_local_fallback', 10, 2 );
	}
	...
	$modernizr_js = ( ualib_is_dev_mode() ) ? '/bower_components/modernizr/modernizr.js' : '/assets/dist' . $modernizr_versioned_js;
	wp_enqueue_script( 'modernizr', get_stylesheet_directory_uri() . $modernizr_js, false, null, false );
	
	wp_enqueue_script( 'jquery' );

	$plugins_js = ( ualib_is_dev_mode() ) ? '/assets/js/plugins.min.js' : '/assets/dist' . $plugins_versioned_js;
	$main_js = ( ualib_is_dev_mode() ) ? '/assets/js/source/main.js' : '/assets/dist' . $main_versioned_js;
	wp_enqueue_script( 'ualib_plugins', get_stylesheet_directory_uri() . $plugins_js, array( 'jquery' ), null, true );
	wp_enqueue_script( 'ualib_main', get_stylesheet_directory_uri() . $main_js, array( 'jquery', 'ualib_plugins' ), null, true );
}

// http://wordpress.stackexchange.com/a/12450
function ualib_jquery_local_fallback( $src, $handle = null ) {
